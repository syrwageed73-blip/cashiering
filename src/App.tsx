...
import { Product, Category, Invoice, StoreSettings, StockLog, CartItem, AppStatePayload, UserProfile, UserRole } from './types';
import { INITIAL_CATEGORIES, INITIAL_PRODUCTS, DEFAULT_SETTINGS } from './initialData';
import { fetchAppState, fetchCurrentUserProfile, saveAppState } from './api';
...
// Premium icons
import { Store, Monitor, HelpCircle, Package, ShieldCheck, ClipboardList, BarChart3, Settings, Database, Keyboard, X, Maximize, Minimize, type LucideIcon } from 'lucide-react';
...
export default function App() {
  ...
  // Always light theme constants (dark theme removed)
  ...
  const currentRole = userProfile?.role || DEFAULT_ROLE;
  ...
  // Update classes for Light Theme only
  useEffect(() => {
  ...
  useEffect(() => {
    if (!isAuthReady || !session || authView === 'recovery') {
      setUserProfile(null);
    ...
    void (async () => {
      try {
        ...
        addToast(t('app.errors.profileLoadFailed'), 'warning');
      } finally {
        if (isMounted) {
          setIsProfileLoading(false);
        }
  ...
  }, [authView, isAuthReady, session?.user.id]);
  ...
  }, [authView, isAuthReady, isProfileLoading, session?.user.id, userProfile?.role]);
  ...
  const handleSignOut = async () => {
    ...
    if (error) {
      ...
      addToast(t('app.errors.signOutFailed'), 'error');
      return;
    }
  ...
  const loadingScreen = (
    <div className="min-h-screen relative overflow-hidden bg-[radial-gradient(circle_at_top_right,_rgba(79,70,229,0.16),_transparent_30%),linear-gradient(135deg,#f8fafc_0%,#eef2ff_45%,#f0f9ff_100%)] flex items-center justify-center p-6">
      ...
      <div className="glass-card-strong rounded-[28px] p-8 text-center relative z-10 max-w-md w-full">
        ...
        <h2 className="mt-5 text-lg font-black text-gray-950">
          {!isAuthReady ? t('app.loading.authCheck') : t('app.loading.dataLoad')}
        </h2>
    ...
    <div className="min-h-screen flex flex-col transition-colors relative">
      ...
      <header className="glass-header sticky top-0 z-[1000] shrink-0 px-4 py-3.5 md:px-5 xl:px-8 print:hidden select-none">
        <div className="mx-auto flex w-full max-w-[1720px] flex-wrap items-center justify-between gap-4 xl:gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-black text-gray-900 leading-tight">{t('app.brand')}</h1>
          ...
          <div className="flex flex-wrap items-center justify-end gap-2.5 xl:max-w-[60%]">
            <span className="hidden lg:inline-flex items-center gap-2 text-xs font-bold text-slate-700 bg-white/70 px-3 py-2 rounded-xl border border-white/70 shadow-sm">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              {session?.user.email}
            </span>
            <span className="hidden lg:inline-flex items-center gap-2 text-xs font-bold text-indigo-700 bg-indigo-50/80 px-3 py-2 rounded-xl border border-indigo-100/50 shadow-sm capitalize">
              {currentRole === 'admin' ? t('app.roles.admin') : t('app.roles.cashier')}
            </span>
            
            {/* Dynamic Clock */}
            <span className="hidden sm:inline-flex items-center gap-1.5 font-mono font-bold text-xs text-indigo-700 bg-indigo-50/80 px-3.5 py-2 rounded-xl border border-indigo-100/50 shadow-sm">
      ...
      <div className="mx-auto flex min-h-0 w-full max-w-[1720px] flex-1 flex-col gap-5 p-4 pb-24 md:p-5 md:pb-5 xl:flex-row xl:items-start xl:gap-6 xl:px-8 xl:pb-8 print:p-0">
        ...
        <aside className="hidden md:flex w-[250px] xl:w-[290px] glass-sidebar p-4 rounded-2xl flex-col justify-between shrink-0 print:hidden select-none animate-fade-in xl:sticky xl:top-[5.75rem] xl:max-h-[calc(100vh-7.25rem)] xl:overflow-y-auto">
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-3.5 py-2.5 text-xs font-semibold rounded-xl transition-all duration-200 cursor-pointer ${
                  activeView === tab.key
                    ...
                    : 'sidebar-item-inactive text-gray-500'
                }`}
              >
          ...
          <div className="mt-6 pt-4 border-t border-indigo-100/30 text-[10px] text-gray-400 text-center leading-relaxed">
            ...
            <div className="relative z-10 flex flex-col xl:flex-row xl:items-end xl:justify-between gap-5">
              <div className="max-w-3xl text-right">
                ...
                <div className="mt-4 flex items-start gap-4">
                  ...
                  <div>
                    <h2 className="text-xl md:text-2xl font-black text-gray-950 leading-tight">{activeNavItem.label}</h2>
                    <p className="text-sm text-gray-600 mt-2 leading-relaxed max-w-2xl">{activeNavItem.description}</p>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 xl:min-w-[460px] 2xl:min-w-[540px]">
                {activeViewSummary[activeView].stats.map((stat) => (
                  <div key={stat.label} className="hero-stat-card rounded-2xl p-4 text-right">
                    <span className="text-[10px] font-bold text-gray-500 block leading-none">{stat.label}</span>
                    <span className="text-sm md:text-base font-black text-gray-950 mt-2 block break-words">{stat.value}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="w-full max-w-md glass-card-strong rounded-2xl p-6 relative text-right modal-content" style={{ direction: 'rtl' }}>
            ...
            <div className="space-y-2 text-xs text-gray-700">
              {[
                ...
                <div key={i} className="flex justify-between items-center py-2 px-3 rounded-xl hover:bg-indigo-50/50 transition-colors border-b border-gray-100/50 last:border-0">
                  <span>{shortcut.label}</span>
                  <kbd className="px-2.5 py-1 bg-gradient-to-b from-gray-50 to-gray-100 rounded-lg font-mono font-bold text-indigo-600 text-[11px] border border-gray-200/80 shadow-sm">{shortcut.key}</kbd>
                </div>
              ))}
            </div>

            <div className="mt-5 pt-3 border-t border-indigo-100/20 text-[10px] text-gray-400 leading-relaxed">
  ...
  if (!isAuthReady || (session && authView !== 'recovery' && (isProfileLoading || isDataLoading))) {
    return loadingScreen;
  }

  const defaultAppPath = `/app/${accessibleViews[0]}`;
  const loginConfigError = hasSupabaseAuthConfig
    ? undefined
